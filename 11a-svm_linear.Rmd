...
Models built on all predictor variables will be saved in `model/svm_linear/all_var/`. 
Models trained on variables selected from RFE in 09 will be saved in `model/svm_linear/rfe_var`. 
...
A polynomial with a linear kernel can only draw a linear boundary. Compared to a polynomial kernel, it has less variance and higher bias. 
