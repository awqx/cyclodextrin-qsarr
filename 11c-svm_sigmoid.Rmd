...
Models built on all predictor variables will be saved in `model/svm_sigmoid/all_var/`. 
Models trained on variables selected from RFE in 09 will be saved in `model/svm_sigmoid/rfe_var`. 

```{r directories}
desc_src <- c("cdk", "mordred", "ochem", "padel")

if (!dir.exists("model")) dir.create("model")
if (!dir.exists("model/svm_sigmoid")) {
  dir.create("model/svm_sigmoid")
  dir.create("model/svm_sigmoid/all_var")
  dir.create("model/svm_sigmoid/rfe_var")
  dir_create_result <- sapply(paste0("model/svm_sigmoid/all_var/", desc_src), dir.create)
  dir_create_result <- sapply(paste0("model/svm_sigmoid/rfe_var/", desc_src), dir.create)
}
```

### General outline for model building

Note that these tuning functions are not as deep as would be required for a project that only uses one set of descriptors. To save time, a very broad tuning grid is used (i.e., with sparse values) and not all possible tuning parameters are implemented. 
...
An SVM with a sigmoid kernel draws a boundary using a `sigmoid` function. It may be better suited for a QSAR than a polynomial kernel because it is less likely to produce extreme values that defy physics. 
...
cdk_all_save <- lapply(
  data_name,
  function(x) {
    saveRDS(cdk_all[[x]], paste0("model/svm_sigmoid/all_var/cdk/", x, ".RDS"))
  }
)

clusterSetRNGStream(cl = cl, 20210927)
padel_all <- parLapply(
  cl = cl,
  padel_list, 
  tune, 
...
padel_all_save <- lapply(
  data_name, 
  function(x) {
    saveRDS(padel_all[[x]], paste0("model/svm_sigmoid/all_var/padel/", x, ".RDS"))
  }
)
...
mordred_all_save <- lapply(
  data_name, 
  function(x) {
    saveRDS(mordred_all[[x]], paste0("model/svm_sigmoid/all_var/mordred/", x, ".RDS"))
  }
)
...
ochem_all_save <- lapply(
  data_name, 
  function(x) {
    saveRDS(ochem_all[[x]], paste0("model/svm_sigmoid/all_var/ochem/", x, ".RDS"))
  }
)
...
cdk_rfe_save <- lapply(
  data_name, 
  function(x) {
    saveRDS(cdk_rfe[[x]], paste0("model/svm_sigmoid/rfe_var/cdk/", x, ".RDS"))
  }
)
...
padel_rfe_save <- lapply(
  data_name, 
  function(x) {
    saveRDS(padel_rfe[[x]], paste0("model/svm_sigmoid/rfe_var/padel/", x, ".RDS"))
  }
)
...
mordred_rfe_save <- lapply(
  data_name, 
  function(x) {
    saveRDS(mordred_rfe[[x]], paste0("model/svm_sigmoid/rfe_var/mordred/", x, ".RDS"))
  }
)
...
ochem_rfe_save <- lapply(
  data_name, 
  function(x) {
    saveRDS(ochem_rfe[[x]], paste0("model/svm_sigmoid/rfe_var/ochem/", x, ".RDS"))
  }
)
stopCluster(cl = cl)
```
...
```{r}
svm_sigmoid_allvar <- lapply(
  paste0("model/svm_sigmoid/all_var/", desc_name), 
  function(x) {
    lapply(
      list.files(x, full.names = T), 
      readRDS
...
ev_allvar <- lapply(
  desc_name, 
  function(x) {
    tune_sublist <- svm_sigmoid_allvar[[x]]
...
```{r}
svm_sigmoid_rfe <- lapply(
  paste0("model/svm_sigmoid/rfe_var/", desc_name), 
  function(x) {
    lapply(
      list.files(x, full.names = T), 
      readRDS
...
ev_rfe <- lapply(
  desc_name, 
  function(x) {
    tune_sublist <- svm_sigmoid_rfe[[x]]
    ev_sublist <- ev_list[[x]]
    lapply(
      data_name, 
      function(y) {
...
```{r}
svm_sigmoid_allvar_df <- rbindlist(
  lapply(
    desc_name, 
    function(x) {
      desc_list <- svm_sigmoid_allvar[[x]]
      rbindlist(
        lapply(
          data_name, 
          function(y) {
...
svm_sigmoid_rfe_df <- rbindlist(
  lapply(
    desc_name, 
    function(x) {
      desc_list <- svm_sigmoid_rfe[[x]]
      rbindlist(
        lapply(
          data_name, 
          function(y) {
...
saveRDS(svm_sigmoid_allvar_df, "result/trn/all_var/svm_sigmoid.RDS")
saveRDS(svm_sigmoid_rfe_df, "result/trn/rfe_var/svm_sigmoid.RDS")
```

```{r, eval = T, echo = F}
svm_sigmoid_allvar_df <- readRDS("result/trn/all_var/svm_sigmoid.RDS")
svm_sigmoid_rfe_df <- readRDS("result/trn/rfe_var/svm_sigmoid.RDS")

ggplot(svm_sigmoid_allvar_df, aes(x = data_src, y = fold_avg, fill = desc_src)) + 
...
ggplot(ev_rfe_ad_temp, aes(x = data_src, y = value, fill = desc_src)) + 
  geom_col(position = "dodge") + 
  facet_grid(summary_stat ~ ., scales = "free")
# svm_sigmoid_tst <- svm_sigmoid_allvar[[1]]
# ev_tst <- ev_list[[1]]
# pred_tst <- lapply(
#   names(svm_sigmoid_tst), 
#   function(x) {
#     # svm_sigmoid_model <- svm_sigmoid_tst[[x]]$model
#     # predict(svm_sigmoid_model, ev_tst[[x]])
#     predict(svm_sigmoid_tst[[x]], ev_tst[[x]])
#   }
# )
```
